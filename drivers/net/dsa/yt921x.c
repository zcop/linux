				   ...
				   unsigned int size)
{
	...
	entries[size] = (typeof(*entries)){};
	entries[size].key[1] = YT921X_ACL_KEYb_TYPE(YT921X_ACL_TYPE_MISC);
	yt921x_acl_entry_set(&entries[size], 1, YT921X_ACL_BINb_MISC_FRAG);

	return size + 1;
}

static struct yt921x_acl_entry *
yt921x_acl_entries_find_misc(struct yt921x_acl_entry *entries,
			     unsigned int size)
{
	for (unsigned int i = 0; i < size; i++)
...
static int
yt921x_acl_entries_parse(struct yt921x_acl_entry *entries, u16 ports_mask,
...
#define entry_prepare() do { \
	...
	*entry = (typeof(*entry)){}; \
	...
	if (flow_rule_match_key(rule, FLOW_DISSECTOR_KEY_TCP)) {
		...
		entry = yt921x_acl_entries_find_misc(entries, size);
		if (!entry) {
			entry_prepare();
			  ...
			  struct yt921x_priv *priv)
{
	...
	res = yt921x_acl_entries_parse(ruleext->r.entries, BIT(port), cls);
	if (res < 0)
		return res;
	ruleext->r.map = (1 << res) - 1;
...
static int
yt921x_acl_commit(struct yt921x_priv *priv, unsigned int entid, u8 entmap)
{
	...
	map = entmap;
	i = 0;
	for_each_set_bit(o, &map, YT921X_ACL_ENT_PER_BLK) {
...
static unsigned int
yt921x_acl_reserve(struct yt921x_priv *priv, unsigned int entries_cnt,
		   struct netlink_ext_ack *extack)
{
	int candidates[YT921X_ACL_ENT_PER_BLK];
	unsigned int acl_used_cnt = 0;

	if (WARN_ON(entries_cnt > YT921X_ACL_ENT_PER_BLK))
		return UINT_MAX;

	for (unsigned int i = 0; i < ARRAY_SIZE(candidates); i++)
		candidates[i] = -1;
	for (unsigned int i = 0; i < YT921X_ACL_BLK_NUM; i++) {
		unsigned int blk_used_cnt = hweight8(priv->acl_maps[i]);
	...
	if (acl_used_cnt + entries_cnt >= YT921X_ACL_NUM)
		goto err;

	for (unsigned int i = YT921X_ACL_ENT_PER_BLK - entries_cnt + 1;
	     i-- > 0;)
		if (candidates[i] >= 0)
...
static int yt921x_acl_del(struct yt921x_priv *priv, unsigned long cookie)
{
	...
	res = yt921x_acl_commit(priv, entid, aclrule->map);
	if (res) {
...
yt921x_acl_add(struct yt921x_priv *priv,
	       ...
	       struct netlink_ext_ack *extack)
{
	unsigned int entries_cnt = hweight8(ruleext->r.map);
	struct device *dev = to_device(priv);
	struct yt921x_acl_rule *aclrule;
	struct yt921x_acl_blk *aclblk;
	...
	aclrule->map = 0;
	blk_used = priv->acl_maps[blkid];
	for_each_clear_bit(o, &blk_used, YT921X_ACL_ENT_PER_BLK) {
		aclrule->map |= BIT(o);
		entries_cnt--;
		if (!entries_cnt)
			break;
